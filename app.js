...
async function loadNews(force = false) {
  ...
  if (allNewsItems.length === 0 || force) grid.innerHTML = '<div class="loading-state"><div class="spinner"></div><p>Fetching latest AI news...</p></div>';
  try {
...
async function fetchRSS(feedUrl, source, label) {
  ...
  return data.items.map((item, i) => ({
    id: 'live-' + source + '-' + i, title: item.title,
    ...
    url: item.link || '#', category: detectCategory(item.title), live: true,
  }));
}
...
function mergeNews(live, curated) {
  const all = [...live, ...curated]; const seen = new Set();
  return all.filter(item => { const k = item.title.slice(0,50).toLowerCase(); if (seen.has(k)) return false; seen.add(k); return true; }).sort((a,b) => new Date(b.date) - new Date(a.date));
}
...
function renderNews() {
  ...
  grid.innerHTML = items.map(n => `<div class="news-card" onclick="window.open('${esc(n.url)}','_blank')">
    <div class="news-card-header"><span class="news-source-badge badge-${n.source}">${esc(n.sourceLabel)}</span><div style="display:flex;align-items:center;gap:6px;">${n.live?'<span style="font-size:10px;color:var(--green);font-weight:600;">● LIVE</span>':''}<span class="news-date">${fmtDate(n.date)}</span></div></div>
    <div class="news-title">${esc(n.title)}</div>
    <div class="news-desc">${esc(n.description)}</div>
    <div class="news-tags">${(n.tags||[]).map(t=>'<span class="tag">'+esc(t)+'</span>').join('')}<span class="tag" style="background:rgba(99,102,241,.1);color:var(--accent);">${esc(n.category)}</span></div>
    <div class="news-footer"><span style="font-size:11px;color:var(--text3);">${n.live?'🌐 Live':'📌 Curated'}</span><a class="read-more" href="${esc(n.url)}" target="_blank" onclick="event.stopPropagation()">Read more →</a></div>
  </div>`).join('');
}
...
function renderTools() {
  ...
  if (searchQuery) tools = tools.filter(t => t.name.toLowerCase().includes(searchQuery) || t.company.toLowerCase().includes(searchQuery) || t.features.some(f => f.toLowerCase().includes(searchQuery)));
  ...
  grid.innerHTML = tools.map(tool => `<div class="tool-card" onclick="openToolModal('${tool.id}')">
    <div class="tool-header"><div class="tool-icon">${tool.icon}</div><div class="tool-meta"><div class="tool-name">${esc(tool.name)}</div><div class="tool-tagline">${esc(tool.company)} · ${esc(tool.tagline)}</div></div><div class="tool-version-badge">v ${esc(tool.currentVersion)}</div></div>
    ...
    <div class="new-features-bar"><div class="new-features-title">🆕 What's New in ${esc(tool.currentVersion)}</div><div class="new-features-list">${tool.newFeatures.slice(0,3).map(f=>'• '+esc(f)).join('<br>')}</div></div>
    <div class="tool-footer"><span class="tool-category-badge cat-${tool.category}">${catLabel(tool.category)}</span><button class="details-btn" onclick="event.stopPropagation();openToolModal('${tool.id}')">Full Details →</button></div>
  </div>`).join('');
}
...
function openToolModal(id) {
  ...
  document.getElementById('modalContent').innerHTML = `
    <div class="modal-tool-header"><div class="modal-tool-icon">${tool.icon}</div><div><div class="modal-tool-name">${esc(tool.name)} <span class="winner-badge">${esc(tool.currentVersion)}</span></div><div class="modal-tool-tagline">${esc(tool.company)} · ${esc(tool.tagline)}</div></div></div>
    <div class="modal-section"><div class="modal-section-title">✅ All Features</div><div class="modal-features-grid">${tool.features.map(f=>'<div class="modal-feature-item">'+esc(f)+'</div>').join('')}</div></div>
    <div class="modal-section"><div class="modal-section-title">🆕 New in ${esc(tool.currentVersion)} vs ${esc(tool.previousVersion)}</div><div class="modal-features-grid">${tool.newFeatures.map(f=>'<div class="modal-feature-item" style="border-left:2px solid var(--green);">'+esc(f)+'</div>').join('')}</div></div>
    <div class="modal-section"><div class="modal-section-title">💰 Pricing Plans</div><div class="modal-pricing-table">${tool.pricing.map((p,i)=>'<div class="modal-pricing-card '+(i===1?'popular':'')+'"><div class="modal-tier-name">'+(i===1?'⭐ ':'')+esc(p.tier)+'</div><div class="modal-tier-price">'+esc(p.price)+'</div><div class="modal-tier-period">'+esc(p.period)+'</div><div class="modal-tier-features">'+p.features.map(f=>'<div class="modal-tier-feature">'+esc(f)+'</div>').join('')+'</div></div>').join('')}</div></div>
    <div class="modal-section"><div class="modal-section-title">📋 Version Diff: ${esc(tool.previousVersion)} → ${esc(tool.currentVersion)}</div><div style="display:flex;flex-direction:column;gap:4px;">${tool.versionDiff.added.map(a=>'<div class="diff-added">+ '+esc(a)+'</div>').join('')}${tool.versionDiff.changed.map(c=>'<div class="diff-changed">~ '+esc(c)+'</div>').join('')}${tool.versionDiff.removed.map(r=>'<div class="diff-removed">- '+esc(r)+'</div>').join('')}</div></div>
    <div class="modal-section"><div class="modal-section-title">🎯 Key Benefits</div><div style="display:flex;flex-direction:column;gap:5px;">${tool.benefits.map(b=>'<div class="benefit-item">'+esc(b)+'</div>').join('')}</div></div>
    <div class="modal-section"><div class="modal-section-title">📊 Performance Scores</div><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:6px;">${Object.entries(tool.score).map(([k,v])=>'<div style="text-align:center;background:var(--surface2);border-radius:8px;padding:10px;"><div style="font-size:20px;font-weight:800;color:'+(v>=9?'var(--green)':v>=7?'var(--orange)':'var(--red)')+';">'+v+'/10</div><div style="font-size:11px;color:var(--text3);text-transform:capitalize;">'+k+'</div></div>').join('')}</div></div>
    <div style="text-align:center;margin-top:16px;"><a href="${esc(tool.website)}" target="_blank" class="cta-btn" style="display:inline-block;text-decoration:none;">Visit ${esc(tool.name)} →</a></div>`;
...
function setupCompare() {
  ...
  AI_TOOLS.forEach(t => { selA.innerHTML += '<option value="'+t.id+'">'+t.name+' ('+t.company+')</option>'; selB.innerHTML += '<option value="'+t.id+'">'+t.name+' ('+t.company+')</option>'; });
  ...
  document.getElementById('compareBtn').addEventListener('click', () => {
    ...
    const sA = Object.values(a.score).reduce((s,v)=>s+v,0), sB = Object.values(b.score).reduce((s,v)=>s+v,0);
...
function renderUseCases() {
  ...
  if (searchQuery) cases = cases.filter(c => c.industryLabel.toLowerCase().includes(searchQuery) || c.department.toLowerCase().includes(searchQuery) || c.tool.toLowerCase().includes(searchQuery));
...
function catLabel(c) { return {'llm':'🧠 LLM','image':'🎨 Image Gen','code':'👨‍💻 Coding','video':'🎬 Video','voice':'🔊 Voice','productivity':'📋 Productivity'}[c]||c; }
function setLastUpdated() { const el=document.getElementById('lastUpdated'); if(el) el.textContent='Last updated: '+new Date().toLocaleTimeString(); }