...
async function fetchLiveNews() {
  // RSS feeds through rss2json API (free, no key needed for basic use)
  const feeds = [
  ...
  const results = await Promise.allSettled(
    feeds.map(f => fetchRSS(f.url, f.source, f.label))
  );
...
function detectTags(text) {
  ...
  const tagMap = {
    ...
    'agent': 'Agents', 'reasoning': 'Reasoning', 'multimodal': 'Multimodal',
    'benchmark': 'Benchmark', 'open source': 'Open Source', 'api': 'API',
    'vision': 'Vision', 'coding': 'Coding', 'research': 'Research',
  };
...
function renderNews() {
  ...
  if (currentNewsFilter !== 'all') {
    items = items.filter(n => n.source === currentNewsFilter);
  }
  if (searchQuery) {
  ...
  if (items.length === 0) {
    grid.innerHTML = `<div class="error-state"><h3>No results found</h3><p>Try a different filter or search term.</p></div>`;
    return;
  }
...
function renderTools() {
  ...
  if (currentToolsFilter !== 'all') {
    tools = tools.filter(t => t.category === currentToolsFilter);
  }
  if (searchQuery) {
  ...
  if (tools.length === 0) {
    grid.innerHTML = `<div class="error-state"><h3>No tools found</h3><p>Try a different filter.</p></div>`;
    return;
  }
  ...
  grid.innerHTML = tools.map(tool => `
    <div class="tool-card" onclick="openToolModal('${tool.id}')">
      ...
      <div class="tool-features">
        ${tool.features.slice(0, 4).map(f => `<div class="feature-item">${escHtml(f)}</div>`).join('')}
      </div>

      <div class="tool-pricing">
        ${tool.pricing.slice(0, 3).map(p => `
          <div class="pricing-tier">
            <div class="tier-name">${escHtml(p.tier)}</div>
            <div class="tier-price">${escHtml(p.price)}</div>
            <div class="tier-desc">${escHtml(p.period)}</div>
          </div>
        `).join('')}
      </div>
...
function openToolModal(id) {
  ...
  content.innerHTML = `
    ...
    <div class="modal-section">
      <div class="modal-section-title">✅ All Features</div>
      <div class="modal-features-grid">
        ${tool.features.map(f => `<div class="modal-feature-item">${escHtml(f)}</div>`).join('')}
      </div>
    </div>

    <div class="modal-section">
      <div class="modal-section-title">🆕 New in ${escHtml(tool.currentVersion)} (vs ${escHtml(tool.previousVersion)})</div>
      <div class="modal-features-grid">
        ${tool.newFeatures.map(f => `<div class="modal-feature-item" style="border-left:2px solid var(--green);">${escHtml(f)}</div>`).join('')}
      </div>
    </div>

    <div class="modal-section">
      <div class="modal-section-title">💰 Pricing Plans</div>
      <div class="modal-pricing-table">
        ${tool.pricing.map((p, i) => `
          <div class="modal-pricing-card ${i === 1 ? 'popular' : ''}">
            <div class="modal-tier-name">${i === 1 ? '⭐ ' : ''}${escHtml(p.tier)}</div>
            <div class="modal-tier-price">${escHtml(p.price)}</div>
            <div class="modal-tier-period">${escHtml(p.period)}</div>
            <div class="modal-tier-features">
              ${p.features.map(f => `<div class="modal-tier-feature">${escHtml(f)}</div>`).join('')}
            </div>
          </div>
        `).join('')}
      </div>
    ...
    <div class="modal-section">
      <div class="modal-section-title">📋 Version Diff: ${escHtml(tool.previousVersion)} → ${escHtml(tool.currentVersion)}</div>
      <div style="display:flex;flex-direction:column;gap:4px;">
        ${tool.versionDiff.added.map(a => `<div class="diff-added">+ ${escHtml(a)}</div>`).join('')}
        ${tool.versionDiff.changed.map(c => `<div class="diff-changed">~ ${escHtml(c)}</div>`).join('')}
        ${tool.versionDiff.removed.map(r => `<div class="diff-removed">- ${escHtml(r)}</div>`).join('')}
      </div>
    </div>
    ...
    <div class="modal-section">
      ...
      <div style="display:flex;flex-direction:column;gap:5px;">
        ${tool.benefits.map(b => `<div class="benefit-item">${escHtml(b)}</div>`).join('')}
      </div>
    </div>
    ...
    <div class="modal-section">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:6px;">
        ${Object.entries(tool.score).map(([k, v]) => `
          <div style="text-align:center;background:var(--surface2);border-radius:8px;padding:10px;">
            <div style="font-size:20px;font-weight:800;color:${v >= 9 ? 'var(--green)' : v >= 7 ? 'var(--orange)' : 'var(--red)'};">${v}/10</div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px;text-transform:capitalize;">${k}</div>
          </div>
        `).join('')}
      </div>
    </div>
    ...
    <div style="text-align:center;margin-top:16px;">
      <a href="${escHtml(tool.website)}" target="_blank" class="cta-btn" style="display:inline-block;text-decoration:none;">Visit ${escHtml(tool.name)} →</a>
    </div>
  `;
...
function renderCompare(a, b) {
  ...
  result.innerHTML = `
    ${renderCompareCol(a, winnerA)}
    ${renderCompareCol(b, !winnerA)}
  `;
  ...
  return `
    <div class="compare-col">
      <div class="compare-col-header">
        <div class="compare-col-icon">${tool.icon}</div>
        <div>
          <div class="compare-col-name">${escHtml(tool.name)} ${isWinner ? '<span class="winner-badge">🏆 Winner</span>' : ''}</div>
          <div style="font-size:12px;color:var(--text2);">${escHtml(tool.company)}</div>
        </div>
      </div>

      <div class="compare-section-title">Latest Version</div>
      <div class="compare-item neutral">${escHtml(tool.currentVersion)} (was ${escHtml(tool.previousVersion)})</div>

      <div class="compare-section-title">Starting Price</div>
      <div class="compare-item pro">${cheapestPaid ? escHtml(cheapestPaid.price) + escHtml(cheapestPaid.period) : 'Free'}</div>

      <div class="compare-section-title">Top Features</div>
      ${tool.features.slice(0, 5).map(f => `<div class="compare-item pro">${escHtml(f)}</div>`).join('')}

      <div class="compare-section-title">New in Latest Version</div>
      ${tool.newFeatures.slice(0, 3).map(f => `<div class="compare-item pro">${escHtml(f)}</div>`).join('')}

      <div class="compare-section-title">Key Benefits</div>
      ${tool.benefits.map(b => `<div class="compare-item neutral">${escHtml(b)}</div>`).join('')}

      <div class="compare-section-title">Performance</div>
      ${Object.entries(tool.score).map(([k, v]) => `
        <div class="compare-item ${v >= 8 ? 'pro' : v >= 6 ? 'neutral' : 'con'}">
          ${k.charAt(0).toUpperCase() + k.slice(1)}: ${v}/10
        </div>
      `).join('')}

      <div style="margin-top:12px;text-align:center;">
        <a href="${escHtml(tool.website)}" target="_blank" class="cta-btn" style="display:inline-block;text-decoration:none;font-size:12px;padding:8px 16px;">Try ${escHtml(tool.name)} →</a>
      </div>
    </div>
  `;
}
  ...
  container.innerHTML = AI_TOOLS.map(tool => `
    <div class="diff-card">
      <div class="diff-header">
        ...
        <span class="diff-tool-name">${escHtml(tool.name)}</span>
        <div class="diff-versions">
          <span class="version-old">${escHtml(tool.previousVersion)}</span>
          <span class="arrow">→</span>
          <span class="version-new">${escHtml(tool.currentVersion)}</span>
        </div>
      </div>
      <div class="diff-body">
        ${tool.versionDiff.added.length ? `
          <div class="diff-section">
            <div class="diff-label">Added</div>
            ${tool.versionDiff.added.map(a => `<div class="diff-added">+ ${escHtml(a)}</div>`).join('')}
          </div>
        ` : ''}
        ${tool.versionDiff.changed.length ? `
          <div class="diff-section">
            <div class="diff-label">Changed / Improved</div>
            ${tool.versionDiff.changed.map(c => `<div class="diff-changed">~ ${escHtml(c)}</div>`).join('')}
          </div>
        ` : ''}
        ${tool.versionDiff.removed.length ? `
          <div class="diff-section">
...
function renderUseCases() {
  ...
  if (currentUsecasesFilter !== 'all') {
    cases = cases.filter(c => c.industry === currentUsecasesFilter);
  }
  if (searchQuery) {
  ...
  if (cases.length === 0) {
    grid.innerHTML = `<div class="error-state"><h3>No use cases found</h3><p>Try a different filter.</p></div>`;
    return;
  }
  ...
  grid.innerHTML = cases.map(uc => `
    <div class="usecase-card">
      <div class="usecase-header">
        <div class="usecase-industry-icon">${uc.icon}</div>
        <div>
          <div class="usecase-industry">${escHtml(uc.industryLabel)}</div>
          <div style="font-size:12px;color:var(--text2);">Real-world AI application</div>
        </div>
        <div class="usecase-dept">${escHtml(uc.department)}</div>
      </div>
      <div class="usecase-body">
        <div class="usecase-tool">
          <span class="usecase-tool-icon">${uc.toolIcon}</span>
          <span class="usecase-tool-name">${escHtml(uc.tool)}</span>
          <span style="font-size:11px;color:var(--text3);margin-left:4px;">→ ${escHtml(uc.department)}</span>
        </div>

        <div class="usecase-scenario">${escHtml(uc.scenario)}</div>

        <div class="usecase-example">
          <div class="usecase-example-label">📌 Real Example</div>
          <div class="usecase-example-text">${escHtml(uc.realExample)}</div>
        </div>

        <div class="usecase-benefits">
          ${uc.benefits.map(b => `<div class="benefit-item">${escHtml(b)}</div>`).join('')}
        </div>

        <div style="font-size:11px;color:var(--text3);margin-bottom:10px;padding:8px;background:var(--surface2);border-radius:6px;">
          📊 <strong style="color:var(--text2);">vs. Traditional:</strong> ${escHtml(uc.comparedTo)}
        </div>

        <div class="usecase-footer">
          <span class="roi-badge">🚀 ${escHtml(uc.roi)}</span>
          <span class="time-badge">⏱️ Saves ${escHtml(uc.timeSaved)}</span>
        </div>
      </div>
    </div>
  `).join('');
